...
flags = tf.app.flags
flags.DEFINE_string('input_folder', '', 'Path to the input folder')
...
# parser.add_argument("-in", "--input_folder", help="input folder for testing")
# parser.add_argument('-f')

# args = parser.parse_args()

# print( "input_folder {} ".format(
#         args.input_folder
#         ))

path = os.path.join(os.getcwd(), FLAGS.input_folder)


CWD = os.getcwd()
# pdf_dir = r"/content/gdrive/My Drive/tensorflow2/models/research/object_detection/images/test"
pdf_dir = os.path.join(CWD, FLAGS.input_folder)
os.chdir(pdf_dir)
...
directory = os.path.join(CWD,FLAGS.input_folder)
...
PATH_TO_TEST_IMAGES_DIR = pathlib.Path(os.path.join(CWD,FLAGS.input_folder))
...
#         print(width,height)
        ...
        df_cord.to_csv('object_cords1.csv')
        
        
        csv_file = pd.DataFrame(pd.read_csv("object_cords1.csv", sep = ",", header = 0, index_col = False))
        csv_file.to_json("object_cords1.json", orient = "records", date_format = "epoch", double_precision = 10, force_ascii = True, date_unit = "ms", default_handler = None)
    ...
    df_count.to_csv('object_counts1.csv')
